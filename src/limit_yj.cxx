...
int f(unsigned ndim, const double * x, void * params, unsigned fdim, double * fval)
{
  ...
  double p_width_free      = heaviside(width_free);
  double p_Rfactor= gaus(Rfactor, integralParams[6], integralParams[7]);

  // Get some other useful variables
  double n_factorial = factorial(integralParams[8]);
  
  double param = (exposure * efficiency * width_free*width_free/Rfactor ) + background;

  // Evaluate Bayesian function
  double bayesian = (exp(-1 * param) * pow(param, integralParams[8])) / n_factorial;

  // Put it all together
  double everything = bayesian * p_exposure * p_efficiency * p_background * p_width_free *p_Rfactor ;
  fval[0] = everything;
...
int main(int argc, char * argv[]) {
  ...
  finalParams.push_back(initialParams[6]);
  //finalParams[6] = R
  finalParams.push_back(initialParams[7]*initialParams[6]);
  //finalParams[7] = uncertainty in R

  if (argc == 11)
    finalParams.push_back(atof(argv[10]));
  else
    finalParams.push_back(finalParams[4]);
  //finalParams[6] (this is n_obs = b ) = val_bkg * mass * val_exp
  for (int i = 0; i<9; i++){
    std::cout << finalParams[i] << std::endl; 
  }
  ...
  xmin[4] = finalParams[6] - (integral_range * finalParams[7]);
  ...
  xmax[4] = finalParams[6] + (integral_range * finalParams[7]);

  if (xmin[1] < 0) xmin[1] = 0;
  if (xmin[2] < 0) xmin[2] = 0;
  if (xmin[3] < 0) xmin[3] = 0;
  if (xmin[4] < 0) xmin[4] = 0;


  if (xmax[2] > 1) xmax[2] = 1;

  double val, err;
  hcubature(1, f, &finalParams, 5, xmin, xmax, 0, 0, 1e-5, ERROR_INDIVIDUAL, &val, &err);
  
  double norm = 1 / val;
  double minimum = 0;
  double maximum = 1.e-7;
